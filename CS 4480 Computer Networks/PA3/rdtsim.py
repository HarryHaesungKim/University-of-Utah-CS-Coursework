...
class EntityA:
    ...
    def __init__(self, seqnum_limit):
        ...
        # Needed for sequence limit and window size.
        self.seqnum_limit = seqnum_limit
        self.window_size = int(seqnum_limit / 2)

        # Needed for implementing GBN.
        self.window = []
        self.packets = []
        self.packets_waiting_for_ack = 0
        self.base = 0
        self.next_seqnum = 0

        self.packet_counter = 0

        # Changes with base incrementation.
        self.seqnums_within_window = []
        for i in range(0, self.window_size):
            self.seqnums_within_window.append(i)

        # Length of time for timer
        # self.time_length = 39.94
        self.time_length = 30
    
    # A helper method that sends a packet that is stored in "memory" to layer3.
    def send_packet(self, packet):
        print("\n-From A, Sending sequence: ", str(packet.seqnum))

        # Send this sucker to layer 3
    ...
    def output(self, message):
        ...
        # Build the packet
        checksum = (2 * (self.packet_counter % self.seqnum_limit)) + sum(message.data)
        packet = Pkt((self.packet_counter % self.seqnum_limit), (self.packet_counter % self.seqnum_limit), checksum, message.data)

        self.packet_counter += 1

        # Populate window / packets list depending on how many packets are currently in the window.
        ...
        # Sending Packet
        if(self.packets_waiting_for_ack < self.window_size):

            self.send_packet(self.window[self.packets_waiting_for_ack])
            self.packets_waiting_for_ack += 1

            if(self.base == self.next_seqnum):
                start_timer(self, self.time_length)
    ...
    def input(self, packet):
        ...
        # For testing:
        # print("From EntityA input: ")
        # print("The seqnum is: " + str(packet.seqnum))
        # # may need to loop seqnum around to accomidate for the limit...
        # print("The acknum is: " + str(packet.acknum))
        # print("The checksum is: " + str(packet.checksum))
        # print("The payload is: " + str(packet.payload))
        # print("The payload sum is: " + str(sum(packet.payload)))

        # Performing checksum on packet.
        checksum = packet.seqnum + packet.acknum + sum(packet.payload)
        
        # If packet is not corrupted.
        if (packet.checksum == checksum):

            print("-From A input, ack received for: " + str(packet.acknum))

            if(packet.acknum in self.seqnums_within_window):

                print("-From A input, ack was within window.")

                # Getting how many packets were acknowledged.
                packets_acked = 0

                if(self.base < packet.acknum):
                    packets_acked = packet.acknum - self.base
                elif(self.base > packet.acknum):
                    packets_acked = (packet.acknum + self.seqnum_limit) - self.base

                packets_acked += 1
                
                # Getting rid of acked packets from the window.
                for i in range(0, packets_acked):
                    self.window.pop(0)
                
                # Populating window.
                while(self.packets and len(self.window) < self.window_size):
                    self.window.append(self.packets.pop(0))

                # Adjusting number of packets being waited on.
                self.packets_waiting_for_ack = self.packets_waiting_for_ack - packets_acked

                # Adjusting window base number.
                self.base = packet.acknum + 1
                if (self.base >= self.seqnum_limit):
                    self.base = 0

                # Adjusting what should be within the window.
                self.seqnums_within_window.clear()
                temp = self.base
                for j in range(temp, temp + self.window_size):
                    self.seqnums_within_window.append(j % self.seqnum_limit)
            ...
            while (self.packets_waiting_for_ack < self.window_size and self.packets_waiting_for_ack < len(self.window)):

                # Note: Using self.packets_waiting_for_ack might not do what we want...
    ...
    def timer_interrupt(self):
        # For testing:
        print("-A timeout has occurred (EntA)! Resending packets starting from: " + str(self.base % self.seqnum_limit))
        # print("The seqnum is: " + str(self.seqnum))
        # print("The acknum is: " + str(self.acknum))
        # print("The checksum is: " + str(self.checksum))
        # print("The payload is: " + str(self.payload) + "\n")

        # Resending all packets within the "window".
        self.packets_waiting_for_ack = 0
        ...
        # Starting timer after sending packets like the TA told me to.
        start_timer(self, self.time_length)
...
class EntityB:
    ...
    def __init__(self, seqnum_limit):
        ...
        # Needed for packets
        self.seqnum = 0
        self.last_good_checksum = 0
        self.last_good_payload = b''

        # Needed for sequence limit and implementing GBN.
        self.seqnum_limit = seqnum_limit
    ...
    def input(self, packet):
        ...
        # For testing:
        # print("-From B, got sequence: " + str(packet.seqnum))
        # # # may need to loop seqnum around to accomidate for the limit...
        # print("The acknum is: " + str(packet.acknum))
        # print("The checksum is: " + str(packet.checksum))
        print("The payload is: " + str(packet.payload) + "\n")

        # Making sure packet is not corrupted via checksum.
        checksum = packet.seqnum + packet.acknum + sum(packet.payload)

        # If checksum is passed, proceed
        if((packet.checksum % checksum) == 0):
            
            # If sequence number is also good, send to layer 5 and acknowledgment to layer 3
            if(packet.seqnum == self.seqnum):

                # For testing:
                print("-From B, packet is good. Sending acknowledgment for sequence: " + str(packet.seqnum))

                # Send to layer 5. Sending to 5 first gets better results?
                to_layer5(self, Msg(packet.payload))

                # Send acknowledgment
                to_layer3(self, Pkt(packet.seqnum, packet.acknum, checksum, packet.payload))

                # Save the good info
                # self.last_good_checksum = checksum
                # self.last_good_payload = packet.payload

                # Increment sequence number
                self.seqnum += 1
                self.seqnum = self.seqnum % self.seqnum_limit
                
                return
            
            else:
                print("-From B, sequence incorrect. Got " + str(packet.seqnum) + " but was expecting " + str(self.seqnum) + ".")
                
                # Send back -1 of the last sequence num incase seqnum ends up being just one above the packets being sent.
                temp_seqnum = self.seqnum - 1
                if temp_seqnum < 0:
                    temp_seqnum = self.seqnum_limit - 1
                temp_checksum = temp_seqnum + temp_seqnum + sum(b'@@@@@@@@@@@@@@@@@@@@')
                to_layer3(self, Pkt(temp_seqnum, temp_seqnum, temp_checksum, b'@@@@@@@@@@@@@@@@@@@@'))

                return

        # sequence number is out of order or packet is corrupted. Discard the packet and send the last good packet/acknowledgement.
        print("-From B, packet was corrupted. Packet discarded. Letting timeout handle it(?).")
        #     to_layer3(self, Pkt(self.seqnum, self.acknum, self.last_good_checksum, self.last_good_payload))

    # Called when B's timer goes off.
    def timer_interrupt(self):
        # Does B even need a timer?
        pass
