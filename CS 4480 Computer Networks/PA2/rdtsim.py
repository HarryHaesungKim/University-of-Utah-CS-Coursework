...
class EntityA:
    ...
    def __init__(self, seqnum_limit):
        self.seqnum, self.acknum, self.checksum, self.seqnum_limit, self.payload, self.wait_for_ack, self.memory = 0, 0, 0, seqnum_limit, b'', False, []
    
    # Sends a packet that is stored in "memory" to layer3.
    def send_packet(self, packet):
        print("\n-From A, Sending sequence: ", str(self.seqnum))
        ...
        # Start timer.
        start_timer(self, 18)
        print("-Time starting: ", get_time(self))
    ...
    def output(self, message):
        ...
        # Need to check if it's waiting for an acknowledgment
        if not (self.wait_for_ack):
            
            # If not waiting for acknowledgment, proceed.
            # Getting the stuff.
            self.wait_for_ack, self.acknum, self.payload = True, 0, self.memory.pop(0)

            # Getting the checksum.
            self.checksum = self.seqnum + self.acknum + sum(self.payload)

            # Send the packet along.
            self.send_packet(Pkt(self.seqnum, self.acknum, self.checksum, self.payload))
        
        # Else:
        # print("".join(["-Waiting for acknowledgment for seq", str(self.seqnum), ". Message ", str(message.data), " stored?."]))
    ...
    def input(self, packet):
        ...
        #Checksum and sequence number match. acknowledgment from EntityB received.
        # NEED TO SEPARATE THIS IF STATEMENT SO THAT acknowledgmentS WORK PROPERLY.
        # Don't need to separate. In EntityA, it doesn't matter as much since if either is off, we resend the packet anyway.
        if((packet.checksum % checksum) == 0 and packet.seqnum == self.seqnum):

            print("-From EntityA input: acknowledgment received from seqnum " + str(packet.seqnum) + ". The time was: " + str(get_time(self)))
            # print("".join(["From EntityA input: acknowledgment received from seqnum ", str(packet.seqnum), "\nThe time was: ", str(get_time(self)), "\n"]))
            ...
            # Not waiting for acknowledgment anymore.
            self.wait_for_ack = False

            # # Going through the stored messages.
            # while(self.memory):
            #     print("Sending sequence (in while loop): ", str(self.seqnum))
            #     self.send_packet()
            ...
            # When receiving acknowledgment from B, send only one packet. Not all at once.

        else:
            # Send packet again.
            print("-Acknowledgment for sequence " + str(self.seqnum) + " checksum failed. Resending packet.")
            self.send_packet(Pkt(self.seqnum, self.acknum, self.checksum, self.payload))
    ...
    def timer_interrupt(self):
        ...
        # For testing:
        print("-A timeout has occurred (EntA)! Resending sequence: " + str(self.seqnum))
...
class EntityB:
    ...
    def __init__(self, seqnum_limit):
        self.seqnum, self.seqnum_limit = 0, seqnum_limit

    # Called from layer 3, when a packet arrives for layer 4 at EntityB.
    # The argument `packet` is a Pkt containing the newly arrived packet.
    def input(self, packet):
        # Should now recieve the packet form EntityA. They should match
        # print("Got sequence from A: ", packet.seqnum)
        ...
        # do the checksum. Simple modulus. Maybe?
        # If the sequence number does not match, or something, don't send the acknowledgment back to EntityA.
        # No. No sequence number matching. Just send back the package. Checksum from EntityA will take care of it all.
        # No. Do the checksum here. If checksum fails, resend
        ...
        if((packet.checksum % checksum) == 0):
            ...
            if(packet.seqnum == self.seqnum):
                ...
                print("-From B, seqnum is off. Resending acknowledgment for sequence: " + str(packet.seqnum))
                
                # Checksum is correct, but the sequence number is off. Send the previous acknowldegement (previous seqnum, everything from packet kept the same).
                resend_seq = self.seqnum - 1
                if(resend_seq < 0):
                    to_layer3(self, Pkt(self.seqnum_limit - 1, 0, packet.checksum, packet.payload))
                else:
                    to_layer3(self, Pkt(resend_seq, 0, packet.checksum, packet.payload))

            return

        # Else, packet arrived corrupted. Let the timeout for EntityA take care of it.
        print("-From B, packet corrupted. Sending nothing. Possible sequence (if not corrupted): ", str(packet.seqnum))
