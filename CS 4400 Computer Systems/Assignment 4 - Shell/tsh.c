...
void eval(char *cmdline)
{
  ...
  if (cmd2 != NULL)
  {
    // Two commands. Pipe set-up needed.

    // First child
    if (fork() == 0)
    {
      // Set up pipe and redirect to the writing end of the pipe.
      dup2(fds[1], 1);
      close(fds[0]);
      close(fds[1]);
    ...
    if ((pid = fork()) == 0)
    {
      // Set up pipe and redirect to the reading end of the pipe.
      dup2(fds[0], 0);
      close(fds[0]);
      close(fds[1]);
  ...
  if (!builtin_cmd(argv1))
  {
    ...
    // Fork to child process to run program.
    if ((pid = fork()) == 0)
    {
      setpgid(0, 0);

      // Unblock signals for sigint and sigtstp
      // sigprocmask(SIG_UNBLOCK, &signalSet, NULL);
      // Unblocking in child process creates bugs.
    ...
    {
      // block signals
      // Block signals only in child process.
      ...
      if (!bg)
      {
        ...
        // sio_puts("I AM NOT A BACKGROUND JOB!\n");

        sigprocmask(SIG_UNBLOCK, &signalSet, NULL);

        // Waiting for forground to be done.
        waitfg(pid);
      }

      // Parent does not wait for background job to terminate
      else
      {
        ...
        // cmdline has a \n character at the end of it already.
        sio_puts(cmdline);

        // unblock signals
        sigprocmask(SIG_UNBLOCK, &signalSet, NULL);

        // Do we do this for background jobs? No I don't think so.
        // waitfg(pid);
      }
    }
  }
...
void do_bg(int jid)
{
  ...
  // getjobjid returns NULL if no job is found.
...
void sigchld_handler(int sig)
{
  // sio_puts("Woah something's happening. I suspect that a child process is done.\n");
  // Traverse all jobs with for-loop.

  for (int i = 0; i < MAXJOBS; i++)
  {

    // Only reaping foreground jobs
    if (jobs[i].state == FG)
    {
      ...
      waitpid(pid, &child_status, WNOHANG | WUNTRACED);
      ...
      else if (WIFSTOPPED(child_status))
      {
        ...
        sio_puts("\n");
        // Don't delete job. It's only stopped.
      }
  ...
  // google to fix in make test06: make: *** [Makefile:36: test06] Interrupt
...
void sigint_handler(int sig)
{
  // sio_puts("You are pressing ctrl-c\n");

  // Loop through jobs to find the right one.
  for (int i = 0; i < MAXJOBS; i++)
  {

    // Only reaping foreground jobs
    if (jobs[i].state == FG || jobs[i].pid == 0)
    {
